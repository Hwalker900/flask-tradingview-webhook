...
def get_technical_analysis(pair):
    try:
        ...
        for attempt in range(3):
            ...
            # Fallback for forex pairs using ccxt
            if pair in {'CADJPY', 'USDHUF', 'USDJPY'}:
                print(f"Falling back to ccxt for {pair}")
                try:
                    exchange = ccxt.binance()
                    symbol = f"{pair[:3]}/{pair[3:]}"
                    ohlcv = exchange.fetch_ohlcv(symbol, timeframe='1d', limit=14)
                    if len(ohlcv) < 14:
                        print(f"Insufficient data from ccxt for {pair}")
                    ...
                    print(f"CCXT fallback failed for {pair}: {e}")
                    return "Neutral", ["📏 Technical analysis unavailable."], 50
            else:
